...
def test_generator_range_does_not_allocate_memory():
    for item in demo_range(100 * (10**9)):
        if item%5 ==1:
            break
    assert True # did you reach here without any memory exception?yes!
...
def test_evens():
    ...
    for num in range(1000):
        assert num*2 == next(gen)

    for num in range(1000):
        assert 2000+num*2 ==next(gen)
...
-Generators are similar to functions which can be used to generate sequences.
-Generators can be helpful in memory constrained situations as they generate values on demand, rather than generating a large number of values at once
-Generators can function similar to iterators and can be converted into a sequence, used as an iterable in a for loop, etc.
"""
