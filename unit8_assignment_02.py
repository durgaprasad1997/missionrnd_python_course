...
def profile_timeit():
    ...
    k = [numbers_string1, numbers_string2,
                       numbers_string3, num_strings4]

    for b in k:

        for index in range(1, 5):

            l = timeit.repeat(stmt='f(' + str(10 ** index) + ')',
                                    setup='from unit8_conversion_methods import ' + b.__name__ + ' as f',
                                    repeat=5, number=1)

            s=10**index
            mini = "%.6f" % min(l)
            t=["%.6f" % j for j in l]
            print(b.__name__ + ", count = " + str(s) + ", min = " + str(mini) + ", actuals = " + str([j for j in t]))
    pass
