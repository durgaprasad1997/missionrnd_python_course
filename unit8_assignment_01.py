...
def profile_perf_counter():
    i = 0
    f = [unit8_conversion_methods.numbers_string1, unit8_conversion_methods.numbers_string2,
    ...
    while i < len(f):
        ...
        for j in range(1, 5):
            ...
            for j in range(0, 5):
                ...
                f[i](c)
                b = time.clock()
                timeTaken.append(b - a)
            minmumTimeTaken = "%0.6f" % min(timeTaken)
            t = ["%.6f" % k for k in timeTaken]
            print(f[i].__name__ + ", count = " + str(c) + ", min = " + minmumTimeTaken + ", actuals = " + str(
                [k for k in t]))
