...
def get_encoding_dict():
    ...
    d = dict(zip(list(string.ascii_lowercase),range(1,27,1)))
    d.update(dict((zip(list(string.ascii_uppercase),range(1,27,1)))))
...
def capitalize_names(input):
    ...
    return list(map(lambda x: x.capitalize(),input))
